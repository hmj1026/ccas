# pdf-secrets-ui Specification

## Purpose

定義 `/setup/secrets` 子頁與 backend `/api/setup/secrets/*` 端點：以
master.key Fernet 加密的 DB `bank_secrets` 表管理 PDF 密碼，並提供
env→DB 一鍵匯入、env fallback、刪除冪等等流程。落地於
oauth-onboarding-ui change（v0.1.0 GA）。
