# gmail-oauth-web Specification

## Purpose

定義 `/setup/gmail` 子頁與 backend `/api/setup/gmail/*` 端點：使用者
於 Web UI 上傳 `credentials.json` 後完成 Google OAuth PKCE 授權流程
（含 state TTL、credentials persistence、revoke、redirect_uri 動態
切換）取代 CLI flow。落地於 oauth-onboarding-ui change（v0.1.0 GA）。
