# installation-quickstart Specification

## Purpose

定義終端使用者從零安裝 CCAS 的純 pull 模式體驗：`docker/docker-compose.yml`
與 `docker/example.env` 範本、`docs/install-quickstart.md` 六步驟流程（含
Gmail OAuth 前置設定 / 首次登入 / 升級 / 誠實揭露仍需手動編輯的設定）。
落地於 compose-pull-deploy change（v0.1.0 GA），未來由
oauth-onboarding-ui 補完手動編輯項目的 UI 化承諾。
