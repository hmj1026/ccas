# bank-management-ui Specification

## Purpose

定義 `/setup/banks` 子頁與 backend `/api/setup/banks` 端點：以 DB
`bank_settings` 表取代手動編輯 `banks.yaml` 作為運行期啟用狀態 SSOT，
UI 提供 toggle、孤兒 badge、聚合統計等流程。落地於 oauth-onboarding-ui
change（v0.1.0 GA）。
