...
定義 CCAS 容器 image 的建置、發布、版本標記與多架構策略。涵蓋 GitHub Actions
release-docker workflow（GHCR 推送 / Release artifact 上傳 / cache）、tag
語意（`vX.Y.Z` / floating `vX.Y` / `vX` / `release`）、以及 backend / frontend /
proxy 三件套的角色分工。落地於 compose-pull-deploy change（v0.1.0 GA）。
