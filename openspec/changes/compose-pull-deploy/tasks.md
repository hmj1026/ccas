...
- [x] 6.5 升級測試：將 `CCAS_VERSION` 從 `v0.1.0-rc.1` 改為 `v0.1.0-rc.2` 後 `docker compose pull && up -d`，確認 alembic migration 自動執行、資料完整保留 *(2026-05-09 path-D upgrade-verify on `/tmp/ccas-upgrade-verify` with `CCAS_PORT=12286`：用既有 GHCR `v0.0.1-test-20260509142409` → `v0.0.2-test-20260509143341` 兩個 release artifact，本機 `docker tag` 重新打為 `v0.0.1` / `v0.0.2`（避開舊 image 內 check-env.sh 不接受 suffix tag 的限制 — 見下文 release blocker）。基準（v0.0.1）：`PUT /api/setup/banks/CTBC enabled=false` + `PUT /api/setup/secrets/CTBC password=upgrade-marker-A`，master.key sha=`fa03760a5eb24773ce95c066ef6a907f1cbcfd01e00da135ec37d32b1e079f62`、api-token-version sha=`6b86b273ff…`、token last4=`3456`、version=1。升級至 v0.0.2 後 backend 自動跑 `==> 套用資料庫 migration` + `alembic.runtime.migration Context impl SQLiteImpl`；CTBC.enabled=False 保留 ✓、CTBC `effective_source=db` 仍 200（Fernet decrypt 用同把 master.key 成功）✓、master.key sha 不變 ✓、api-token-version sha 不變 ✓、token-info `last4=3456 version=1` 不變 ✓。**Release blocker finding**：v0.0.1/v0.0.2 image 內舊版 check-env.sh regex 為 `^(release\|v\d+(\.\d+){0,2})$`，會拒絕含 `-rc.1` / `-test-20260509…` suffix 的 tag；develop 已修為 `^(release\|local\|v\d+(\.\d+){0,2}(-[A-Za-z0-9.]+)?)$`（`scripts/check-env.sh:89`），但必須先把 develop merge master 並推一個新版 release image 後，才可以推 `v0.1.0-rc.1` 正式 tag — 否則 §7.2 推完 RC tag、終端使用者用同一 RC tag 跑 compose 會被舊 image baked-in 的 regex 卡住。)*
- [x] 6.6 多架構測試：在 Mac M 系列實機 pull `linux/arm64` variant 並啟動成功 *(2026-05-09 path-D：`docker manifest inspect ghcr.io/hmj1026/ccas-{backend,frontend,proxy}:v0.0.2-test-20260509143341` 三個 image 均含 `linux/amd64 + linux/arm64` 雙 platform；本機 Mac M `docker image inspect …:v0.0.2 --format '{{.Architecture}}'` 三個 image 全 `arm64`；§6.5 stack 全程在 arm64 variant 下 healthy。)*
...
>
> **Pending 原因（2026-05-09 path-D）**：使用者政策「等全部開發 merge master 才發第一個版本」+ §6.5 發現的
