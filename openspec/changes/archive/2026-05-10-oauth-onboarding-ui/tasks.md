...
- [x] 13.4 pdf-secrets 端對端：DB 設定密碼 → 跑 pipeline 解密成功；刪除 DB 條目 + env 仍存在 → 解密成功（fallback）；刪除 DB + 刪除 env → 解密失敗 + 明確錯誤訊息 *(2026-05-09 path-D 端對端 on `/tmp/ccas-openspec-verify` with `CCAS_PORT=8080`：自造加密 PDF fixture（pymupdf AES-256 + owner_pw=user_pw=`test-decrypt-pwd-13-4`、size=1.5K，validated `head -c8000 | grep '/Encrypt'` + `pymupdf.open().authenticate(...)=6`），分兩個 bank dir：TESTBANK（.env 含 `PDF_PASSWORD_TESTBANK`）+ TESTBANK_NOPW（無 env）。直接呼叫 `run_decryption_job(session, options=PipelineOptions(bank_code=...))`（避開 SQLite lock 的 pipeline orchestrator）三路驗證：**Path (a) DB**：`PUT /api/setup/secrets/TESTBANK {password: ...}` → resolve_passwords 回 1 candidate（DB Fernet 解出）→ `decrypted=1 passthrough=0 failed=0`、status=decrypted ✓；**Path (b) env fallback**：`DELETE /api/setup/secrets/TESTBANK` 後重建加密 PDF → resolve_passwords 回 1 candidate（env 來源）+ DB BankSecret row 不存在 → `decrypted=1 failed=0`、status=decrypted ✓；**Path (c) 無 DB 無 env**：TESTBANK_NOPW dir → resolve_passwords 回 0 candidates → `decrypted=0 failed=1`、status=decrypt_failed、error_reason=`[Decrypt] PDF 解密失敗: Password not found in settings` ✓。Cleanup：刪 TESTBANK/TESTBANK_NOPW staging dirs + DB rows + .env 還原。)*
...
- [x] 13.8 redirect_uri 變更：將 `CCAS_PORT` 從 8080 改 12283 → 進 `/setup/gmail` 看到提示新 redirect URI → GCP Console 同步後可完成授權 *(2026-05-09 path-D 真實授權 on `/tmp/ccas-openspec-verify`：把 `.env` 從 `CCAS_PORT=8080 / PUBLIC_BASE_URL=http://localhost:8080` 切到 `CCAS_PORT=12283 / PUBLIC_BASE_URL=http://localhost:12283` 後 `up -d`。`GET /api/setup/gmail/authorize` 回傳的 `redirect_uri=http://localhost:12283/setup/gmail/callback` 已隨 PUBLIC_BASE_URL 動態切換 ✓。**因 credentials.json 仍是同一個 GCP Desktop client（`ccas-492008`），Google Desktop loopback policy 允許任意 localhost port + path，不需要在 GCP Console 加新 redirect URI**（與 §13.2 同樣的 Desktop policy 解釋）。使用者重新 login + 點「授權 Google」後，backend 收到 `GET /api/setup/gmail/callback?code=4%2F0AeoWuM_7a9PHuZY5S0bpTDcD9oG58YJZIW37pKZIyb_ZOszzVBqVDuTySh1h9WcTlpC-iQ&state=MkmD...&...` 回 `303 See Other` ✓、`data/token.json` 0600 寫入、`GET /api/setup/gmail/status connected=true granted_scopes=["…/gmail.readonly"]` ✓。Revoke `POST /api/setup/gmail/revoke` 200 → token.json 已刪 → `connected=false` ✓。Tear down 後 `.env` 還原至 8080 / PUBLIC_BASE_URL=http://localhost:8080。如果未來改用 GCP **Web** type client（redirect_uri 嚴格比對），切 port 時會拒簽 `redirect_uri_mismatch`，需先在 GCP Console Authorized redirect URIs 加入 `http://localhost:<NEW_PORT>/setup/gmail/callback`，這個前置在 docs/install-quickstart §自訂 port 段落需明示。)*
...
> - §13.4 已於 path-D 用自造 AES-256 加密 PDF fixture 直接跑 `run_decryption_job` 驗證 DB / env /
>   none 三路徑全綠（避開 SQLite lock 的 pipeline orchestrator）。
> - §13.8 已於 path-D 用 `CCAS_PORT=12283 / PUBLIC_BASE_URL=http://localhost:12283` 切換 + 同一 GCP
>   Desktop client 走 Google loopback any-port policy 完成完整 authorize → consent → callback →
>   connected → revoke 全鏈路驗證，已勾選。Web client 用法在 evidence 補充。
...
> - §14.3 archive：等 compose-pull-deploy archive 後才執行（compose §7.x release tag pending）。
...
- [x] 14.3 完成後 `/opsx:archive oauth-onboarding-ui`，確認 delta 同步至 `openspec/specs/` *(2026-05-10 path-D-2 v0.1.0 GA 後執行；compose-pull-deploy archive 先行於本 change（依 §14.2 落地順序）)*
