...
`UbotV1Parser.can_parse()` SHALL 透過 PDF 首頁文字特徵辨識聯邦銀行信用卡帳單。

#### Scenario: 辨識聯邦銀行帳單 PDF
- **WHEN** 輸入 PDF 首頁包含「聯邦銀行」與「信用卡」關鍵字
- **THEN** `can_parse()` SHALL 回傳 `True`

#### Scenario: 拒絕非聯邦帳單 PDF
- **WHEN** 輸入 PDF 首頁不包含聯邦銀行特徵關鍵字
...
`UbotV1Parser.parse()` SHALL 從聯邦帳單 PDF 提取所有消費交易明細。
