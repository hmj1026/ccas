# 無 Docker 的 Worker 與 Scheduler 維運

本文件管理不使用 Docker Compose 時的 CCAS background process。範圍只有：
...
Backend API、frontend、Telegram bot 與 MCP server 不由這組 service manager 管理。
...
安裝會根據目前作業系統選擇 service manager，並把目前 checkout 的絕對路徑與
uv 路徑寫入 user-level service definition：
...
install 會啟動 worker 與 scheduler，並設定失敗自動重啟：worker/scheduler 在
...
可以連到 queue，以及 scheduler heartbeat 在最近兩分鐘內更新：
...
3. 確認 API_TOKEN、DATABASE_URL、REDIS_URL 在 repository root .env，並且
   執行 uv sync --frozen 後再重試。
4. 確認 backend/data/scheduler-heartbeat 可寫入；若設定了
