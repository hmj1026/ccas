...
| Agent MCP | `ccas-mcp` / `python -m ccas.mcp` | 官方 MCP SDK stdio 唯讀工具介面；serverInfo version 與 package metadata 同步 |
...
Agent surfaces 共用 `ccas.services` 的安全投影；REST 提供 `/api/bills/payment-due` 與 `/api/pipeline/status`，CLI/MCP 僅允許唯讀查詢。Agent response DTO 的 datetime 在 JSON serialization boundary 統一輸出 UTC RFC3339 `Z`；`AGENT_WRITE_ENABLED` 預設為 false，且目前不會暴露任何寫入工具。
