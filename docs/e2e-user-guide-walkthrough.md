...
| 7 | TAISHIN | S2 decrypt | 部分歷史帳單 decrypt 失敗：`Invalid password`（樣本：`TSB_Creditcard_Estatement_2020{01..10}.pdf`）。最近帳單 decrypt 成功（bills=16 / tx=222）。疑似 TAISHIN 舊版 PDF 密碼格式與現行 `PDF_PASSWORD_TAISHIN` 不同。 | fix-taishin-parser-historical-password | spec-ready |
| 8 | FUBON | S1 ingest | **部分結案**：SPA 遷移 + schema drift（`code` → `errorMsg/jwt`）+ JWT redaction 已修，unit test 綠。真實 pipeline 實跑又曝露 3 個獨立新問題，拆成 #9/#10/#11 分別追蹤。 | research-fubon-web-fetch-pipeline → impl-fubon-fetcher-pipeline | partially archived |
