...
Docker Compose 以開發模式啟動（含 override，6 個服務）：
- **Backend**: uvicorn with `--reload`、tesseract OCR 已內建
- **Worker**: RQ 2.x worker，跑 pipeline / classifier / notifier
...
> 對應的 prod release 部署（`docker/docker-compose.yml`）會多一個 `proxy`（nginx reverse proxy）作為對外入口，dev 模式不啟用。
