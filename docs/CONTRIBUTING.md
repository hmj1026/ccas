...
<!-- AUTO-GENERATED: scripts table — 由 scripts/ 與 frontend/package.json 抽出，請勿手動編輯 -->

### Repo scripts（`./scripts/*.sh`）
...
| Script | 用途 |
|--------|------|
| `pnpm dev` | Vite dev server（含 `/api` proxy） |
| `pnpm build` | `tsc -b && vite build` |
| `pnpm lint` | ESLint |
| `pnpm test` | Vitest 單元測試（`src/**`，**不含** `e2e/**`） |
| `pnpm test:watch` | Vitest watch 模式 |
| `pnpm e2e` | Playwright E2E（`frontend/e2e/*.spec.ts`） |
