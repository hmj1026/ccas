"""CTBC v1 parser integration tests using synthetic PDFs.

Uses fpdf2 to generate PDFs that mimic CTBC statement layout,
...
def _create_non_ctbc_pdf(path: Path, font_path: Path) -> Path:
    """Generate a non-CTBC statement PDF."""
    pdf = _new_pdf(font_path)
    pdf.add_page()
...
def _create_multi_page_ctbc_pdf(path: Path, font_path: Path) -> Path:
    """Generate a multi-page CTBC statement PDF."""
    pdf = _new_pdf(font_path)
    ...
    pdf.cell(0, 8, "帳單月份：2026年03月", new_x="LMARGIN", new_y="NEXT")
    pdf.cell(0, 8, "繳費截止日：2026/04/15", new_x="LMARGIN", new_y="NEXT")
    pdf.cell(0, 8, "本期應繳總額：NT$ 535", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(5)
    _write_table_row(pdf, _TABLE_HEADERS)
    _write_table_row(pdf, ("03/01", "03/03", "1234", "全聯福利中心", "350"))

    # Page 2: another transaction
    pdf.add_page()
    _write_table_row(pdf, _TABLE_HEADERS)
    _write_table_row(pdf, ("03/05", "03/07", "1234", "台灣大車隊", "185"))

    pdf.output(str(path))
    return path


class TestCtbcV1PdfIntegration:
    def _get_parser(self):
        from ccas.parser.banks.ctbc_v1 import CtbcV1Parser

        return CtbcV1Parser()

    def test_can_parse_synthetic_ctbc_pdf(self, tmp_path, cjk_font_path):
        parser = self._get_parser()
        pdf_path = _create_ctbc_pdf(tmp_path / "ctbc.pdf", cjk_font_path)

        assert parser.can_parse(pdf_path) is True

    def test_cannot_parse_non_ctbc_pdf(self, tmp_path, cjk_font_path):
        parser = self._get_parser()
        pdf_path = _create_non_ctbc_pdf(tmp_path / "cathay.pdf", cjk_font_path)
    ...
    def test_parse_returns_valid_result(self, tmp_path, cjk_font_path):
        ...
        assert result.total_amount == 925
    ...
    def test_parse_multi_page_pdf(self, tmp_path, cjk_font_path):
        parser = self._get_parser()
        pdf_path = _create_multi_page_ctbc_pdf(
            tmp_path / "ctbc_multi.pdf", cjk_font_path
        )

        result = parser.parse(pdf_path)

        assert result.total_amount == 535
...
class TestCtbcRegistryIntegration:
    def test_import_banks_registers_ctbc_v1(self):
        from ccas.parser.banks.ctbc_v1 import CtbcV1Parser

        registry.register(CtbcV1Parser())
        candidates = registry.resolve("CTBC")

        assert len(candidates) >= 1
        assert candidates[0].bank_code == "CTBC"
        assert candidates[0].version == "v1"

    def test_module_level_registration_on_fresh_import(self):
        """Verify module-level registration works (without autouse clear)."""
        from ccas.parser.banks.ctbc_v1 import CtbcV1Parser

        parser = CtbcV1Parser()
        registry.register(parser)

        versions = registry.get_versions("CTBC")
