...
def _create_ctbc_pdf(path: Path, *, include_transactions: bool = True) -> Path:
...
def _create_non_ctbc_pdf(path: Path) -> Path:
...
def _create_multi_page_ctbc_pdf(path: Path) -> Path:
...
class TestCtbcV1PdfIntegration:
    ...
    def test_can_parse_synthetic_ctbc_pdf(self, tmp_path):
        parser = self._get_parser()
        pdf_path = _create_ctbc_pdf(tmp_path / "ctbc.pdf")

        assert parser.can_parse(pdf_path) is True

    def test_cannot_parse_non_ctbc_pdf(self, tmp_path):
        parser = self._get_parser()
        pdf_path = _create_non_ctbc_pdf(tmp_path / "cathay.pdf")
    ...
    def test_parse_multi_page_pdf(self, tmp_path):
        parser = self._get_parser()
        pdf_path = _create_multi_page_ctbc_pdf(tmp_path / "ctbc_multi.pdf")
