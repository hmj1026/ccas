...
def _create_non_cathay_pdf(path: Path, font_path: Path) -> Path:
...
def _create_multi_page_cathay_pdf(path: Path, font_path: Path) -> Path:
...
class TestCathayV1PdfIntegration:
    ...
    def test_can_parse_synthetic_cathay_pdf(self, tmp_path, cjk_font_path):
        parser = self._get_parser()
        pdf_path = _create_cathay_pdf(tmp_path / "cathay.pdf", cjk_font_path)

        assert parser.can_parse(pdf_path) is True

    def test_cannot_parse_non_cathay_pdf(self, tmp_path, cjk_font_path):
        parser = self._get_parser()
        pdf_path = _create_non_cathay_pdf(tmp_path / "ctbc.pdf", cjk_font_path)
    ...
    def test_parse_multi_page_pdf(self, tmp_path, cjk_font_path):
        parser = self._get_parser()
        pdf_path = _create_multi_page_cathay_pdf(
            tmp_path / "cathay_multi.pdf", cjk_font_path
        )
