"""SinoPac v1 parser integration tests using synthetic PDFs.

Uses fpdf2 to generate PDFs that mimic SinoPac statement layout,
...
def _create_sinopac_pdf(path: Path, *, include_transactions: bool = True) -> Path:
    ...
    pdf.cell(0, 8, "SinoPac Bank Credit Card Statement", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(5)
    pdf.cell(0, 8, "2026年03月份帳單", new_x="LMARGIN", new_y="NEXT")
    pdf.cell(0, 8, "繳費截止日：2026/04/20", new_x="LMARGIN", new_y="NEXT")
    pdf.cell(0, 8, "本期應繳總額：NT$ 4,050", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(5)
    ...
    if include_transactions:
        ...
        for row in [
            ...
            ("03/08", "03/10", "5678", "家樂福", "1280"),
            ("03/15", "03/17", "9012", "momo購物網", "2350"),
        ]:
...
def _create_multi_page_sinopac_pdf(path: Path) -> Path:
    ...
    pdf.cell(0, 8, "2026年03月份帳單", new_x="LMARGIN", new_y="NEXT")
    pdf.cell(0, 8, "繳費截止日：2026/04/20", new_x="LMARGIN", new_y="NEXT")
    pdf.cell(0, 8, "本期應繳總額：NT$ 1,700", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(5)
    _write_table_row(pdf, _TABLE_HEADERS)
    _write_table_row(pdf, ("03/01", "03/03", "5678", "全聯福利中心", "420"))

    # Page 2: another transaction
    pdf.add_page()
    _write_table_row(pdf, _TABLE_HEADERS)
    _write_table_row(pdf, ("03/08", "03/10", "5678", "家樂福", "1280"))

    pdf.output(str(path))
    return path


class TestSinopacV1PdfIntegration:
    ...
    def test_parse_returns_valid_result(self, tmp_path):
        ...
        assert result.total_amount == 4050
...
class TestSinopacRegistryIntegration:
    def test_import_banks_registers_sinopac_v1(self):
        from ccas.parser.banks.sinopac_v1 import SinopacV1Parser

        registry.register(SinopacV1Parser())
        candidates = registry.resolve("SINOPAC")

        assert len(candidates) >= 1
        assert candidates[0].bank_code == "SINOPAC"
    ...
    def test_module_level_registration_on_fresh_import(self):
        ...
        parser = SinopacV1Parser()
        registry.register(parser)

        versions = registry.get_versions("SINOPAC")
