...
def _create_taishin_pdf(path: Path, *, include_transactions: bool = True) -> Path:
...
def _create_non_taishin_pdf(path: Path) -> Path:
...
def _create_multi_page_taishin_pdf(path: Path) -> Path:
...
class TestTaishinV1PdfIntegration:
    ...
    def test_can_parse_synthetic_taishin_pdf(self, tmp_path):
        parser = self._get_parser()
        pdf_path = _create_taishin_pdf(tmp_path / "taishin.pdf")

        assert parser.can_parse(pdf_path) is True

    def test_cannot_parse_non_taishin_pdf(self, tmp_path):
        parser = self._get_parser()
        pdf_path = _create_non_taishin_pdf(tmp_path / "cathay.pdf")
    ...
    def test_parse_multi_page_pdf(self, tmp_path):
        parser = self._get_parser()
        pdf_path = _create_multi_page_taishin_pdf(tmp_path / "taishin_multi.pdf")
