"""Taishin v1 parser integration tests using synthetic PDFs.

Uses fpdf2 to generate PDFs that mimic Taishin statement layout,
...
def _create_taishin_pdf(
    path: Path, font_path: Path, *, include_transactions: bool = True
) -> Path:
    """Generate a synthetic Taishin statement PDF."""
    pdf = _new_pdf(font_path)
    pdf.add_page()

    pdf.cell(0, 10, "台新銀行 信用卡帳單", new_x="LMARGIN", new_y="NEXT")
    pdf.cell(0, 8, "Taishin Bank Credit Card Statement", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(5)
    pdf.cell(0, 8, "2026年03月份帳單", new_x="LMARGIN", new_y="NEXT")
    pdf.cell(0, 8, "繳費截止日：2026/04/22", new_x="LMARGIN", new_y="NEXT")
    pdf.cell(0, 8, "本期應繳總額：NT$ 3,270", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(5)
    ...
    if include_transactions:
        ...
        for row in [
            ...
            ("03/10", "03/12", "6789", "好市多", "2360"),
            ("03/18", "03/20", "1234", "NETFLIX.COM", "390"),
        ]:
...
def _create_non_taishin_pdf(path: Path, font_path: Path) -> Path:
    """Generate a non-Taishin statement PDF."""
    pdf = _new_pdf(font_path)
    pdf.add_page()
...
def _create_multi_page_taishin_pdf(path: Path, font_path: Path) -> Path:
    """Generate a multi-page Taishin statement PDF."""
    pdf = _new_pdf(font_path)
    ...
    pdf.cell(0, 8, "2026年03月份帳單", new_x="LMARGIN", new_y="NEXT")
    pdf.cell(0, 8, "繳費截止日：2026/04/22", new_x="LMARGIN", new_y="NEXT")
    pdf.cell(0, 8, "本期應繳總額：NT$ 1,800", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(5)
    _write_table_row(pdf, _TABLE_HEADERS)
    _write_table_row(pdf, ("03/03", "03/05", "6789", "全聯福利中心", "520"))

    # Page 2: another transaction
    pdf.add_page()
    _write_table_row(pdf, _TABLE_HEADERS)
    _write_table_row(pdf, ("03/10", "03/12", "6789", "好市多", "1280"))

    pdf.output(str(path))
    return path


class TestTaishinV1PdfIntegration:
    def _get_parser(self):
        from ccas.parser.banks.taishin_v1 import TaishinV1Parser

        return TaishinV1Parser()

    def test_can_parse_synthetic_taishin_pdf(self, tmp_path, cjk_font_path):
        parser = self._get_parser()
        pdf_path = _create_taishin_pdf(tmp_path / "taishin.pdf", cjk_font_path)

        assert parser.can_parse(pdf_path) is True

    def test_cannot_parse_non_taishin_pdf(self, tmp_path, cjk_font_path):
        parser = self._get_parser()
        pdf_path = _create_non_taishin_pdf(tmp_path / "cathay.pdf", cjk_font_path)
    ...
    def test_parse_returns_valid_result(self, tmp_path, cjk_font_path):
        ...
        assert result.total_amount == 3270
        assert result.due_date == date(2026, 4, 22)
        assert len(result.transactions) == 3
        assert result.transactions[0].merchant == "全聯福利中心"
        assert result.transactions[0].amount == 520

    def test_parse_multi_page_pdf(self, tmp_path, cjk_font_path):
        parser = self._get_parser()
        pdf_path = _create_multi_page_taishin_pdf(
            tmp_path / "taishin_multi.pdf", cjk_font_path
        )
...
class TestTaishinRegistryIntegration:
    def test_import_banks_registers_taishin_v1(self):
        from ccas.parser.banks.taishin_v1 import TaishinV1Parser

        registry.register(TaishinV1Parser())
        candidates = registry.resolve("TAISHIN")

        assert len(candidates) >= 1
        assert candidates[0].bank_code == "TAISHIN"
    ...
    def test_module_level_registration_on_fresh_import(self):
        ...
        versions = registry.get_versions("TAISHIN")
