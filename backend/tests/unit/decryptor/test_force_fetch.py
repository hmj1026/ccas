"""Force 模式下 fetch_pending_attachments 擴大查詢範圍的測試。"""
...
from ccas.decryptor.staging import fetch_pending_attachments
...
class TestForceFetch:
    async def test_normal_mode_returns_staged_only(self, db_session):
        db_session.add(_make_attachment("staged", attachment_id="a1"))
        db_session.add(
            _make_attachment("decrypted", message_id="m2", attachment_id="a2")
        )
        db_session.add(
            _make_attachment("decrypt_failed", message_id="m3", attachment_id="a3")
        )
    ...
    async def test_force_mode_includes_decrypted_and_failed(self, db_session):
        db_session.add(_make_attachment("staged", attachment_id="a1"))
        db_session.add(
            _make_attachment("decrypted", message_id="m2", attachment_id="a2")
        )
        ...
        db_session.add(_make_attachment("parsed", message_id="m4", attachment_id="a4"))
        await db_session.flush()

        options = PipelineOptions(force=True)
        results = await fetch_pending_attachments(db_session, options)
        statuses = {r.status for r in results}
        assert statuses == {"staged", "decrypted", "decrypt_failed"}
        assert len(results) == 3

    async def test_force_mode_excludes_parsed(self, db_session):
