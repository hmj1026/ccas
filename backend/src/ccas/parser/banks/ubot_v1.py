...
# Exact "本期應繳總額" tier: matched before the looser fallback so a stray
# 「本期最低應繳金額」 cannot be picked up when both appear.
_RE_TOTAL_AMOUNT_EXACT = re.compile(r"本期應繳總額[：:]?\s*(?:NT\$?\s*)?([\d,]+)")
# Looser fallback for layout variants. ``(?<!最低)`` rejects the substring
# 「應繳金額」 inside 「本期最低應繳金額」 (mirrors cathay_v1's guard) so the
# minimum-due figure is never mistaken for the total payable.
_RE_TOTAL_AMOUNT = re.compile(
    r"(?<!最低)(?:本期)?應繳[總金][額額][：:]?\s*(?:NT\$?\s*)?([\d,]+)"
)
...
class UbotV1Parser(BankParser):
    ...
    def _extract_total_amount(self, text: str) -> int | None:
        ...
        match = _RE_TOTAL_AMOUNT_EXACT.search(text)
        if match:
            return int(match.group(1).replace(",", ""))
