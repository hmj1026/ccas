...
# 本期應繳總額：NT$ 12,345 (forces 本期 prefix so 前期應繳總額 cannot win).
# Suffix alternation matches 總金額 / 總額 / 金額 (mirrors esun_v1's literal).
_RE_TOTAL_AMOUNT_REAL = re.compile(
    r"本期應繳(?:總金額|總額|金額)[：:]?\s*(?:NT\$?\s*)?([\d,]+)"
)
# Fallback: 應繳總額 / 應繳金額 without 本期 prefix (older / simplified layouts).
# Negative lookbehind on 前期 stops the bare 應繳總額 inside 前期應繳總額 from
# winning when no 本期 label exists (mirrors cathay_v1's (?<!最低) guard).
_RE_TOTAL_AMOUNT = re.compile(
    r"(?<!前期)(?:本期)?應繳(?:總金額|總額|金額)[：:]?\s*(?:NT\$?\s*)?([\d,]+)"
)
...
class FubonV1Parser(BankParser):
    ...
    def _extract_total_amount(self, text: str) -> int | None:
        ...
        Prefers the 本期-prefixed label so a 前期應繳總額 line earlier in the
        document cannot be matched first; falls back to the generic label for
        layouts that omit the 本期 prefix.
        """
