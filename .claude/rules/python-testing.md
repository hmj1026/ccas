...
`ccas-tdd-red-check.sh` auto-runs `test_*.py` on Write to confirm RED. Coverage gate: `uv run pytest --cov --cov-report=term-missing`. For deeper TDD guidance use the installed `tdd-workflow` runtime skill.
